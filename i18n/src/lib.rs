...
impl I18nString {
    fn get_field(&self, locale: Locale) -> &Option<String> {
        match locale {
            en_US => &self.en_US,
            zh_CN => &self.zh_CN,
        }
    ...
    fn get_field_mut(&mut self, locale: Locale) -> &mut Option<String> {
        match locale {
    ...
    pub fn set(&mut self, slice: &str, locale: Option<Locale>) {
        if let Some(locale) = locale {
            *self.get_field_mut(locale) = Some(slice.to_string());
        } else {
